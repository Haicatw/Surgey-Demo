...
{
    public bool cameraMovementActivated = false;
    ...
    void Update()
    {
        ...
        {
            cameraMovementActivated = !cameraMovementActivated;
        }
        if (cameraMovementActivated)
        {
            // float speed = (moveSpeed + (Input.GetAxis("Fire3") * shiftAdditionalSpeed));
            this.gameObject.transform.Translate(Vector3.forward * moveSpeed * Input.GetAxis("Vertical"));
            this.gameObject.transform.Translate(Vector3.right * moveSpeed * Input.GetAxis("Horizontal"));
